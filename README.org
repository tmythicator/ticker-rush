...
- *Backend:* Go (Golang) + Gin Framework. Implements a "Producer-Consumer" pattern:
  ...
  - *API:* HTTP handler serving data and real-time updates via **Server-Sent Events (SSE)**.
  - *Security:* **Authorization Middleware** (JWT) ensures secure access to simulated trading and portfolio management.
- *Data Layer:*
  - *PostgreSQL:* Primary store for user profiles, portfolios, and transaction history.
  - *Valkey (Redis fork):* High-performance cache for hot market data and quotes.
...
- [-] *Phase 2: Gamification*
  - [ ] Leaderboard system
  - [X] User Profiles & Transaction History
  - [X] "Sell All" Position with Real-Time Price Confirmation
