...
func TestLoadConfig_Defaults(t *testing.T) {
	...
	assert.Equal(t, 3*time.Second, cfg.FetchInterval)
...
func TestLoadConfig_Overrides(t *testing.T) {
	...
	require.NoError(t, os.Setenv("FETCH_INTERVAL", "500ms")) // Test duration parsing
	require.NoError(t, os.Setenv("SLEEP_INTERVAL", "1s"))
	...
	assert.Equal(t, 500*time.Millisecond, cfg.FetchInterval)
	assert.Equal(t, 1*time.Second, cfg.SleepInterval)
