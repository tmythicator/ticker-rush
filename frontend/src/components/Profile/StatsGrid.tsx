    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl reltive overflow-hidden group">
                ...
                <div className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-2">Total Net Worth</div>
                <div className="text-4xl font-bold font-mono tracking-tight">
                    ${totalNetWorth.toFixed(2)}
                </div>
                <div className="mt-4 flex items-center gap-2 text-sm text-slate-300">
                    <span className="bg-slate-800 px-2 py-1 rounded-lg">Cash: ${user.balance.toFixed(2)}</span>
                    <span className="bg-slate-800 px-2 py-1 rounded-lg">Assets: ${investedCapital.toFixed(2)}</span>
                </div>
            </div>
